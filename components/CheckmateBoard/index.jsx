...
const ExportModal = memo(function ExportModal({
  ...
  async function download() {
    const blob = new Blob([string], {
      type:
        mode === "pgn" ? "application/x-chess-pgn" : "application/x-chess-fen",
    });
    ...
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Body>
        <ButtonToolbar className={"mb-2"} aria-label={"Share options"}>
          {allowShare ? (
            <Button onClick={share} className={"me-2"} variant={"primary"}>
              Share
            </Button>
          ) : undefined}
          <Button
          ...
          {allowDownload ? (
            <Button onClick={download} className={"me-2"} variant={"secondary"}>
              Download
            </Button>
          ) : undefined}
        </ButtonToolbar>
...
const BoardPlaceholder = memo(function BoardPlaceholder({
  text,
  chessboardSize,
}) {
  const style = {
...
export default function CheckmateBoard({
  storedgame,
  game,
  onDrop,
  whiteTimer,
  ...
  return (
    <Container className={"d-flex flex-row mb-3"}>
      ...
      <Dropdown className={"mb-2"} id={styles.shareDropdown}>
        ...
        <Dropdown.Menu>
          <Dropdown.Item
            onClick={onExportClicked("link")}
            href="#"
            disabled={!storedgame?.players}
          >
            As Link
          </Dropdown.Item>
          <Dropdown.Item onClick={onExportClicked("pgn")} href="#">
            As .PGN
          </Dropdown.Item>
          <Dropdown.Item onClick={onExportClicked("fen")} href="#">
            As .FEN
          </Dropdown.Item>
        </Dropdown.Menu>
      </Dropdown>
      ...
      <div
        id={styles.moveBoard}
        className={"p-2 m-2 mw-75 bg-dark flex-fill rounded text-white"}
      >
        <Container>
          <div className="row row-cols-2">
            {storedgame?.players ? (
              <>
                <div
                  id="whitePlayer"
                  className={
                    "col chessMove align-self-start bg-white text-dark text-center align-items-center justify-content-center d-flex"
                  }
                >
                  <UserCard
                ...
                <div
                  id="blackPlayer"
                  className={
                    "col chessMove align-self-end bg-secondary text-center align-items-center justify-content-center d-flex"
                  }
                >
                  <UserCard
            ...
            <div
              id="whiteTimer"
              className={
                "col chessMove align-self-start bg-white text-dark text-center"
              }
            >
              Time:&nbsp;
              <b>
            ...
            <div
              id="blackTimer"
              className={
                "col chessMove align-self-end bg-secondary text-center"
              }
            >
              Time:&nbsp;
              <b>
            ...
            {moves.length > 0
              ? moves.map((move, index) => {
                  let moveclass = whiteMove
                    ? "col chessMove align-self-start text-center"
                    : "col chessMove align-self-end text-center";
                  whiteMove = !whiteMove;
                  return (
                    <div key={move + index + whiteMove} className={moveclass}>
                      {move}
                    </div>
                  );
                })
