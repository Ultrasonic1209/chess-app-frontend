...
function getOpponent(players, is_white) {
  if (typeof players === "undefined" || typeof is_white === "undefined") {
    return {};
  }
...
function formatPlayer(player) {
  if (typeof player.is_white === "undefined") {
    return "None yet";
  }
...
export default function LoadGame() {
  ...
  useEffect(() => {
    ...
    if (gamemode === "NET") {
      ...
      if (isNaN(parseInt(presence))) {
        // this will fire if presence querystring is "" or nonexistent
        router.replace({
  ...
  const gamemodeOnClick = async (ev) =>
    await router.replace({
      query: { ...router.query, gamemode: ev.target.dataset.gamemode },
    });
  const presenceOnClick = async (ev) =>
    await router.replace({
  ...
  const pageOnClick = async (ev) =>
    await router.replace({
      query: {
        ...router.query,
        page: ev.target.dataset.page || ev.target.parentNode.dataset.page,
      },
    });
  ...
  const { data, error, isValidating } = useSWR(
    gamemode === "NET" && router.isReady
      ? "https://apichessapp.server.ultras-playroom.xyz/chess/get-games/?" +
          params.toString()
      : null,
    fetcher,
    { fallbackData: [] }
  ...
  const amountOfGames =
    gamemode === "NET" ? data?.length || 0 : games?.length || 0;
            ...
            {gamemode === "NET"
                  ...
                  <tr key={game.game_id}>
                    ...
                    <td>
                      {game.is_white === null
                        ? "N/A"
                        : formatPlayer(
                            getOpponent(game?.players, game?.is_white)
                          )}
                    </td>
                    <td>
                      {game.time_ended
              ...
              : games?.map((game) => (
                  <tr key={game.id}>
                    ...
                    <td>{game.gameWon ? game.gameWon : "None yet"}</td>
                    <td>
                      <Button
                        variant="primary"
                        href={
                          "/game/" + game.gameType.toLowerCase() + "/" + game.id
                        }
                        as={Link}
                      >
                        Enter Game
