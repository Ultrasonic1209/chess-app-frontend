...
export default function Profile() {
  ...
  if (error) {
    ...
    return <Main title="Sign in"></Main>;
  } else {
    const handleSignOut = async () => {
      setLoggingOut(true);
      await fetch(
        "https://apichessapp.server.ultras-playroom.xyz/user/logout",
        {
          method: "DELETE",
    ...
    return (
      <Main title="Profile">
        <UpdateUserPrompt
          show={userPromptOpen}
          handleClose={() => setUserPromptOpen(false)}
        />
        <h2>Profile</h2>
        ...
        />
        <p className={"mb-0"}>
          Want to customise your avatar? Create a{" "}
          <Link href={"https://gravatar.com/"}>Gravatar</Link>
        </p>
        <small>
          Custom avatars will only be displayed if they are rated PG.
        </small>
      </Main>
    );
  }
