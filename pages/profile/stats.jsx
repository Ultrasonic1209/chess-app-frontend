...
export default function Stats() {
  ...
  const { data: netstats, error: neterror } = useSWR(
    "https://apichessapp.server.ultras-playroom.xyz/user/stats/",
    fetcher
  );
  ...
  const globalQuery = useLiveQuery(async () => {
    ...
    const amountOfGamesWon = await db
      .table("games")
    ...
    const amountOfGamesWhite = await db
      .table("games")
      .where({
        colourPlaying: "WHITE",
      })
  ...
  const localQuery = useLiveQuery(async () => {
    if (typeof gamemode === "undefined" || gamemode === "NET") {
      return [];
    }
    ...
    const amountOfGamesWon = await games
      .filter(function (game) {
        return game.colourPlaying === game.gameWon;
      })
      .count();
    ...
    return {
      ...
      percentage_of_starting_white:
        (amountOfGamesWhite / Math.max(amountOfGames, 1)) * 100,
    };
  }, [gamemode]);
  ...
  return (
    <Main title="Stats">
      ...
      {globalQuery && netstats ? (
        <GlobalStats
          games_played={
            globalQuery.amountOfGames +
            netstats.games_played +
            " (" +
            netstats.games_played +
            " online)"
          }
          games_won={
            globalQuery.amountOfGamesWon +
            netstats.games_won +
            " (" +
            netstats.games_won +
            " online)"
          ...
          percentage_of_playing_white={
            (
              (globalQuery.amountOfGamesWhite /
                Math.max(globalQuery.amountOfGames, 1)) *
              100
            ).toFixed(2) +
            "% offline, " +
            (netstats.percentage_of_playing_white || 0).toFixed(2) +
            "% online"
          }
        />
        ...
        <Tab eventKey="BOT" title="Bot">
          {localQuery && gamemode != "NET" ? (
            <>
              ...
              <p>
                Games won: {localQuery.games_won} (
                {(
                  (localQuery.games_won /
                    Math.max(localQuery.games_played, 1)) *
                  100
                ).toFixed(2)}
                %)
              </p>
              <p>
                Percentage of games played as white:{" "}
                {(localQuery.percentage_of_starting_white || 0).toFixed(2)}%
              </p>
            </>
          ) : (
            <b>Loading</b>
        ...
        <Tab eventKey="NET" title="Online">
          {netstats && gamemode === "NET" ? (
            <>
              ...
              <p>
                Games won: {netstats.games_won} (
                {(
                  (netstats.games_won / Math.max(netstats.games_played, 1)) *
                  100
                ).toFixed(2)}
                %)
              </p>
